 ...
 const [activeIndex, setActiveIndex] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchProgramDetails = async () => {
      try {
        const response = await fetch(`https://certfillapi.reckonio.com/api/programs/${id}`, {
          headers: {
      ...
      console.log(data)
        setProgram({ name: data.name, description: data.description, courses: data.courses });
        setLoading(false);
      } catch (err: any) {
  ...
  return (
    <div className="h-screen overflow-hidden min-h-[700px] flex flex-row">
      ...
      <div className="h-full grow bg-white p-20 text-black flex flex-col justify-between">
        ...
        <div className="mx-auto max-w-[700px] w-full"> {program && (
          <div className="space-y-5 py-8">
            {
            program?.courses.map((course)=>(

              <div className="flex flex-col gap-3" key={course._id} onClick={() => toggleCourse(course._id)}>
                <div className="text-2xl flex-row justify-between flex">
                   <span>{course.name}</span>
                <span className="text-xl">
                  {activeIndex === course._id ? <FaChevronDown/> : <FaChevronUp/>}
                  </span>
                </div>
        ...
        <button
            className="mainButton   h-[68px] capitalize"
            type="submit"
          >
            save
          </button></div>
