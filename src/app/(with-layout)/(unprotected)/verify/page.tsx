...
export default function VerifyUser() {
  ...
  const handleSubmit = (formState: UseFormReturn) => async (data: Record<string, any>) => {
    try {
      const response = await customFetch<{ message: string }>(`/auth/verify-email`, {
        method: 'POST',
  ...
  return (
    <>
      <div className="text-center flex flex-col gap-5 py-10 px-5">
        <h3 className="text-3xl font-bold  text-neptuneSDream">Verify your email </h3>
        <p className="text-cloakGrey text-sm text-center font-light">
          We have sent a code to <strong className="block font-semibold">{email}</strong>
        </p>
